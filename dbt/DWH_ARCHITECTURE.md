...
ClickHouse EnergyHub использует современную многослойную архитектуру Data Warehouse (DWH) с четким разделением ответственности между слоями. Архитектура адаптирована для локального ClickHouse кластера.

### 🚀 Кластерная архитектура

**Локальный кластер** состоит из:
...
graph TB
    ...
    subgraph "ClickHouse Cluster (Local)"
        subgraph "Shard 1"
...
ALTER TABLE table_name ON CLUSTER default ADD COLUMN ...;

-- Удаление данных на кластере
ALTER TABLE table_name ON CLUSTER default DELETE WHERE ...;
```
